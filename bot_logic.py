...
class TradingBot:
    ...
    def _rebalancear_ordenes_binance(self, config):
        ...
        # 3. Calcular capital por orden: usar el tamaño de orden configurado
        tamano_orden = calcular_tamano_posicion(config)
        
        # Capital máximo disponible para órdenes
        usdc_en_ordenes = sum(o['usdc'] for o in ordenes_binance)
        capital_disponible = usdc_en_ordenes + saldo_libre
        
        if capital_disponible < 5:
            return
        
        # Cada orden recibe tamano_orden, salvo que no haya suficiente
        capital_necesario = tamano_orden * total_ordenes
        if capital_necesario > capital_disponible:
            # No hay suficiente para dar tamano_orden a cada una → repartir lo que hay
            capital_por_orden = capital_disponible / total_ordenes
        else:
            capital_por_orden = tamano_orden
        ...
        self.log(f"⚖️ Rebalanceando {total_ordenes} órdenes: {capital_por_orden:.2f} USDC cada una (total: {capital_por_orden * total_ordenes:.2f})", "INFO")
